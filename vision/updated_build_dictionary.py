...
def preprocess_template(path):
    ...
    # --- THE SHADOW KILLER ---
    # Artificially boost brightness to wash out light shadows before Otsu sees them.
    blur = cv2.add(blur, 30)

    # Auto threshold for lighting differences
    _, thresh = cv2.threshold(
        blur, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU
    )

    # Clean up small noise / reconnect small breaks
    kernel = np.ones((3, 3), np.uint8)
    thresh = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel)
    thresh = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, kernel)

    # Find all white pixels directly
    ...
    # Ignore tiny junk by checking bbox area
    ...
    # Preserve aspect ratio: fit into a square canvas
    h, w = cropped.shape
...
for path in files:
    ...
    if processed is None:
        print(f"Skipped {name}: could not isolate ink cleanly")
        continue
